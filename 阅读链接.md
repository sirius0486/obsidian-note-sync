...
import { Card, Text, ConstructKit, H2, H3, MuseoSans } from '@construct-kit/core';
import styled, { createGlobalStyle } from 'styled-components';
import { getConfigByClient } from '../../config';
import {MainSiteLink} from "../../components/mainSiteLink";
import {Header} from "../../components/header";

const urlSearchParams = new URLSearchParams(window.location.search);
const config = getConfigByClient(urlSearchParams.get('client_id'));

const LogoView = () => {
  const { rootRedirectUri, siteName} = config;
  ...
  return   (
    <StyledHeader logo={config.logos.headingLogo} logoAlt={''} href={rootRedirectUri}>
      {config.logos.headingLogo}
      <H3> {siteName} </H3>
    </StyledHeader>
  );
}

const ReturnMainSiteLink = () => {
  const { rootRedirectUri, siteName } = config;
  const returnLinkContent =  `Return to ${ siteName }`
  return (
    <MainSiteLink rootRedirectUri={rootRedirectUri} returnLinkContent={returnLinkContent}  />
  );
}

const Logo = styled(LogoView)`
  margin-top: 0.2rem;
  margin-bottom: 1rem;
  line-height: 25px;
  display: flex;
  ...
  return(
    <ConstructKit>
      ...
      <Box>
        ...
        <Text>The page you’re trying to access has restricted access. If you feel this is a mistake, contact support.</Text>
        <ReturnMainSiteLink  />
      </Box>
    </ConstructKit>
  );
